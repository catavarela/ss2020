...
public class Universe {
    ...
    public double calculate(double final_t, double delta_t, Metodo metodo) {
        ...
        while (current_t < final_t) {
            ...
            if (hay_cohete && !no_fue_agregado && ((crashed_body = rocketCrash(current_t)) != null)) {
                ...
                System.out.println("La nave chocó contra " + crashed_body.getName() + " en t=" + current_t);
            }
    ...
    private Body rocketCrash(double t) {
        ...
        for (Body b : celestial_bodies) {
            if (b.getName().equals("Cohete")) cohete = b;
        }

        if (cohete == null) return null;
    ...
    private Body addRocket(double t) {
        ...
        double v_tan_mod = rocket_blastoff_velocity + orbital_velocity_of_space_station + Math.hypot(tierra.getV(t)[0], tierra.getV(t)[1]);
        double v_tan_x = tierra.getV(t)[0] / Math.hypot(tierra.getV(t)[0], tierra.getV(t)[1]);
        double v_tan_y = tierra.getV(t)[1] / Math.hypot(tierra.getV(t)[0], tierra.getV(t)[1]);

        vx0 = v_tan_mod * v_tan_x;
        vy0 = v_tan_mod * v_tan_y;
